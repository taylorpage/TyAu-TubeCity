...
private let log = Logger(subsystem: "com.taylor.audio.TubeCityExtension", category: "AudioUnitViewController")

@MainActor
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<TubeCityExtensionMainView>?
	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try TubeCityExtensionAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? TubeCityExtensionAudioUnit else {
				log.error("Unable to create TubeCityExtensionAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(TubeCityExtensionParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = TubeCityExtensionMainView(parameterTree: observableParameterTree)
